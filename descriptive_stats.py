def generate_descriptive_statistics(df):
    ...
    print(f"\nGender Distribution:")
    print(df['gender'].value_counts(normalizeTrue).map('{:.1%}'.format))
    
    print(f"\nRace/Ethnicity Distribution:")
    print(df['race_ethnicity'].value_counts(normalizeTrue).map('{:.1%}'.format))
    
    print(f"\nEducation Distribution:")
    print(df['education'].value_counts(normalizeTrue).map('{:.1%}'.format))
    ...
    print(df['offense_type'].value_counts(normalizeTrue).map('{:.1%}'.format))
    
    print(f"\nOffense Severity Distribution:")
    print(df['offense_severity'].value_counts(normalizeTrue).map('{:.1%}'.format))
